...
export default function AddProduct() {
  ...
  const fileHandleChange = (event) => {
    ...
    newFiles.forEach(file=>{
      if(file?.size > 1000000){
        ...
        newErrs.push(`${file?.name} is not and image`);
      }
    })
    setFiles(newFiles);
    setFileErrs(newErrs);
  ...
  //select data from store
  // brands: {brands} เท่ากับ const brands = state?.brands?.brands;
  const {brands: {brands}} = useSelector(
    (state) => state?.brands);

  //colors
  const [colorsOption, setColorOption] = useState([]);
  //select data from store
  // colors: {colors} เท่ากับ const colors = state?.colors?.colors;
  ...
  return (
    <>
      ...
      {fileErrs?.length > 0 && files?.length == 0 ? (<ErrorMsg message="file not found"/>) : null}
      {fileErrs?.length > 0 && files?.length > 0 ? (<ErrorMsg message="file too large or upload an image"/>) : null }
