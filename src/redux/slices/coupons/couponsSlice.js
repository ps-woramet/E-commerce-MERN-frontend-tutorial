...
export const fetchCouponsAction = createAsyncThunk('coupons/fetch All', async(payload, {rejectWithValue, getState, dispatch}) => {
    try{
...
//fetch coupon action
export const fetchCouponAction = createAsyncThunk('coupon/fetch Single', async(code, {rejectWithValue, getState, dispatch}) => {
    try{
...
//reset coupon action
export const resetCouponAction = createAsyncThunk('coupon/reset', async(code, {rejectWithValue, getState, dispatch}) => {
    try{
        return code
    }catch(error){
        return rejectWithValue(error?.response?.data);
...
const couponsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        //reset Coupon
        builder.addCase(resetCouponAction.pending, (state) => {
            state.loading = true;
        });
        builder.addCase(resetCouponAction.fulfilled, (state, action) => {
            state.loading = false;
            state.coupon = action.payload;
            state.isAdded = false
        });
        builder.addCase(resetCouponAction.rejected, (state, action) => {
            state.loading = false;
            state.coupon = null;
            state.isAdded = false
            state.error = action.payload;
